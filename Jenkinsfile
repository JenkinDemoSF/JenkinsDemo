...
node {
	...
	def SOURCE_PATH = env.SOURCE_PATH
		 ...
		 rc = bat returnStatus: true, script: "\"${toolbelt}\"sfdx force:auth:logout --targetusername ${HUB_ORG} -p"
			...
			if (isUnix()) {
			   ...
			   //rmsg = bat returnStdout: true, script: "\"${toolbelt}\"sfdx force:mdapi:deploy -d manifest/. -u ${HUB_ORG}"
				rmsg = bat returnStdout: true, script: "\"${toolbelt}\"sfdx force:source:deploy -p 'C:\ProgramData\Jenkins\.jenkins\workspace\Basic-Demo_QA\force-app\main\default' -u ${HUB_ORG}"
			}
